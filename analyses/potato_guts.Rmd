...
```{r NMDS_A_and_B, echo = F, fig.width = 10, fig.height = 8}
...
cap_model_full <- capscale(gut_mat_species ~ BCTV + Year +
                             region + host_plant + sample_date,
                           data = gut_matrix, distance = "bray")
anova.cca(cap_model_full, by = "term", permutations = 999)
summary(cap_model_full)

# Extract scores
site_scores_full <- scores(cap_model_full, display = "sites") %>%
  as.data.frame() %>%
  mutate(sample = rownames(.), bctv = gut_matrix$BCTV)

species_scores_full <- scores(cap_model_full, display = "species") %>%
  as.data.frame() %>%
  rownames_to_column("species")

# Filter for plotting
species_scores_full_filtered <- species_scores_full %>%
  filter(abs(CAP1) > 0.2 | abs(CAP2) > 0.2) 

ggplot(site_scores_full,
       aes(x = CAP1, y = CAP2, color = bctv, fill = bctv)) +
  stat_ellipse(geom = "polygon", type = "t", level = 0.95, alpha = 0.3) +
  ...
  geom_text(data = species_scores_full_filtered,
            aes(x = CAP1 * 1.1, y = CAP2 * 1.1, label = species),
            size = 3, hjust = 0.5,
            inherit.aes = F) +
  scale_color_manual(values = c("0" = "#1b9e77", "1" = "#d95f02")) +
  scale_fill_manual(values = c("0" = "#1b9e77", "1" = "#d95f02")) +
